...
public class StudentDAO {
    ...
    public void deleteStudent(int id) throws Exception {
        try (Connection con = DBConnection.getConnection()) {
            ...
            if (rs.next()) {
                ...
                // Step 3: Delete from users (student will be deleted automatically via ON DELETE CASCADE)
    ...
    public void deleteAllStudents() throws Exception {
        try (Connection con = DBConnection.getConnection()) {
            // Step 1: Delete from class_students first to avoid FK constraint issue
            con.prepareStatement("DELETE FROM class_students").executeUpdate();

            // Step 2: Get all Users_id from student table before it's auto-deleted
            PreparedStatement getUsersStmt = con.prepareStatement("SELECT Users_id FROM student");
            ResultSet rs = getUsersStmt.executeQuery();

            // Step 3: Delete from users table (this will cascade delete students because of FK with ON DELETE CASCADE)
            while (rs.next()) {
                int userId = rs.getInt("Users_id");
                PreparedStatement deleteUserStmt = con.prepareStatement("DELETE FROM users WHERE id=?");
                deleteUserStmt.setInt(1, userId);
    ...
    public void updateTotalFine(int studentId) throws Exception {
        ...
        // ✅ Corrected query: no status column, update `is_struck_off` and `struck_off_date`
    ...
    public void markParentMeetingDone(int studentId) throws Exception {
        String updateStudentQuery = "UPDATE student SET is_struck_off = FALSE, parent_meeting_done = TRUE, struck_off_date = NULL WHERE id = ?";
        String deleteAbsentsQuery = "DELETE FROM Attendance_Register WHERE student_id = ? AND status = 'Absent'";

        try (Connection con = DBConnection.getConnection();
             PreparedStatement ps1 = con.prepareStatement(updateStudentQuery);
             PreparedStatement ps2 = con.prepareStatement(deleteAbsentsQuery)) {
    ...
    public List<StudentModel> getStruckOffStudents() throws Exception {
        List<StudentModel> list = new ArrayList<>();
        String query = "SELECT * FROM student WHERE is_struck_off = TRUE";
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                student.setRollNo(rs.getString("roll_no"));
                student.setBatch(rs.getString("batch"));
                student.setStruckOffDate(rs.getDate("struck_off_date"));
                list.add(student);
            }
        }
        return list;
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    student.setRollNo(rs.getString("roll_no"));
                    student.setBatch(rs.getString("batch"));
                    student.setName(rs.getString("student_name")); // ✅ Name from users table
                }
            }
